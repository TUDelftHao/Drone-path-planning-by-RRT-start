...
u(1)=para.m*norm(attr); % u(1) = F1+F2+F3+F4;
...
u(4)=para.Iz*saturate(angleDelta(psi_p,psi)+0-vpsi); % T2+T4-T1-T3;
u(2)=20*para.Ix*(u(1)+u(4))/2*saturate(angleDelta(phi_p,phi)+0-1*vphi)/para.l; % F4-F2;
u(3)=20*para.Iy*(u(1)-u(4))/2*saturate(angleDelta(theta_p,theta)+0-1*vtheta)/para.l; % F3 - F1;
